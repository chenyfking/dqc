<template>
  <el-card>
    ...
    <el-table v-loading="tableLoading" element-loading-text="数据加载中..." :data="list">
      ...
      <el-table-column prop="ruleCount" label="规则数量"></el-table-column>
      ...
      <el-table-column prop="onlineReqSuccessRatio" label="联机调用次数">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      params.pageNum = 10;
      this.$axios.get("/report/summary", { params }).then(res => {
    ...
    exportReport() {
      ...
      this.$axios({
        method: "get",
        url:`/report/exportSummary?keywords=${params.keywords}&startDate=${params.startDate}&endDate=${params.endDate}` ,
        responseType: "blob",
      }).then((res) => {
        if (res.data.type == "application/json") {
          ...
          a.href = url;
          a.target = "_blank";
          a.setAttribute("download", '汇总报表.xlsx');
