<template>
  <el-card>
    ...
    <el-table v-loading="tableLoading" element-loading-text="数据加载中..." :data="list">
      ...
      <el-table-column prop="batchReqCount" label="调用次数"></el-table-column>
      <el-table-column prop="batchReqSuccessRatio" label="调用通过率">
        <template slot-scope="scope">
          {{scope.row.batchReqSuccessRatio ? scope.row.batchReqSuccessRatio + "%" : "-"}}
        </template>
      </el-table-column>
      <el-table-column prop="onlineReqCount" label="调用次数"></el-table-column>
      <el-table-column prop="onlineReqSuccessRatio" label="调用通过率">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      params.pageNum = 10;
      this.$axios.get("/report/detail", { params }).then(res => {
    ...
    exportReport() {
      ...
      this.$axios({
        method: "get",
        url:`/report/exportDetail?keywords=${params.keywords}&startDate=${params.startDate}&endDate=${params.endDate}` ,
        responseType: "blob",
      }).then((res) => {
        if (res.data.type == "application/json") {
          ...
          a.href = url;
          a.target = "_blank";
          a.setAttribute("download", '明细报表.xlsx');
