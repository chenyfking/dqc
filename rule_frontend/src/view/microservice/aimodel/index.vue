<template>
  <div>
    <el-card>
      ...
      <el-table
        v-loading="tableLoading || opLoading"
      ...
      >
        <el-table-column label="模型名称">
          <template slot-scope="scope">
            <span v-if="scope.row.enable">{{scope.row.modelName}}</span>
            <el-tooltip v-else placement="top" content="已禁用">
              <span style="text-decoration: line-through">{{scope.row.modelName}}</span>
            </el-tooltip>
          </template>
        </el-table-column>
        <el-table-column prop="jarName" label="模型文件" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column sortable="custom" prop="updateTime" label="更新时间"></el-table-column>
        <el-table-column sortable="custom" prop="createTime" label="创建时间"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="text" @click="microDoalogInfo(scope.row)">查看被引用服务</el-button>
            <dropdown @command="moreOperation" class="moreOperation">
              ...
              <dropdown-menu slot="dropdown">
                <dropdown-item
                  ...
                  v-if="$hasPerm('aimodel:edit')"
                  icon="el-icon-edit"
                >编辑</dropdown-item>
                <dropdown-item
                  :command="{row: scope.row, op: 'enable'}"
                  v-if="$hasPerm('aimodel:edit')"
                  :icon="scope.row.enable ? 'el-icon-lock' : 'el-icon-unlock'"
                >{{scope.row.enable ? '禁用' : '启用'}}</dropdown-item>
                <dropdown-item
                  :command="{row: scope.row, op: 'download'}"
                  v-if="$hasPerm('aimodel:download')"
                  icon="el-icon-download"
                >下载模型文件</dropdown-item>
                <dropdown-item
                  :command="{row: scope.row, op: 'downloadParam'}"
                  v-if="$hasPerm('aimodel:download')"
                  icon="el-icon-download"
                >下载参数文件</dropdown-item>
                <dropdown-item
    ...
    >
      <el-form :model="form" :rules="rules" ref="form" label-width="80px" @submit.native.prevent>
        ...
        <el-form-item label="模型文件" prop="jarName">
          <el-upload
            action
...
export default {
  ...
  methods: {
      ...
      this.$axios({
        ...
        .then(res => {
          if (res.data.type == "application/json") {
            ...
            a.href = url;
            a.target = "_blank";
            a.setAttribute("download", row.className + ".csv");
            document.body.appendChild(a);
            a.click();
          }
        })
    ...
    initAccounts() {
      this.$axios.get("/user/search?page=-1").then(res => {
        if (res.data.code == 0) {
